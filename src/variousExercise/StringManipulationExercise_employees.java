...
public class StringManipulationExercise_employees {
	...
	public static void main(String[] args) {
		...
		String[] employees = { "Philipa Salthouse [Electrical Engineer] psalthouse0@g.co",
				...
				"Marthena Otson [Librarian] motsonh@wsj.com", "Nehemiah MacRitchie [VP Sales] nmacritchiei@prweb.com",
		...
		for (String each : employees) {
			String jobTitle = each.substring(each.indexOf("[") + 1, each.indexOf("]"));
		...
		for (String each : employees) {
			String nameFull = each.substring(0, each.indexOf(" ["));
			String jobTitle = each.substring(each.indexOf("[") + 1, each.indexOf("]"));
			System.out.println(nameFull + " - " + jobTitle);
		}
		// TASK 3. Look for "Tracy Bartle" should be Geological Engineer. Check for that...
		...
		for (String each : employees) {
			...
			if (nameFull.equalsIgnoreCase("Tracy Bartle") && jobTitle.equalsIgnoreCase("Geological Engineer")) {
				System.out.println("PASS: Tracy Bartle is Geological Engineer");
				break;
			} else {
				System.out.println("FAIL: Tracy Bartle is " + jobTitle);
				break;
			}
